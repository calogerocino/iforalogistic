...
interface VTCPartner {
  name: string;
  logoUrl: string;
  websiteUrl?: string; // Opzionale, se vuoi linkare il logo o il nome
}
...
export class PublicHomepageComponent implements OnInit {
  ...
  toggleRegistrationPanel(): void {
    ...
    if (this.isRegistrationPanelOpen) {
      this.updateAvailableSlots(
        this.vtcRegistrationForm.get('participantsCount')?.value || 1
      );
    }
  }
  ...
  updateAvailableSlots(participantsNeeded: number | null): void {
    if (
      !this.selectedEventForModal ||
      !this.selectedEventForModal.slots ||
    ...
    this.availableSlotsForBooking = this.selectedEventForModal.slots.filter(
      (slot) => {
        const bookedPlaces =
    ...
    const selectedSlotIdControl =
      this.vtcRegistrationForm.get('selectedSlotId');
    if (
      selectedSlotIdControl?.value &&
      !this.availableSlotsForBooking.find(
        (s) => s.id === selectedSlotIdControl.value
      )
    ) {
  ...
  async onVtcRegisterSubmit(): Promise<void> {
    if (this.vtcRegistrationForm.invalid || !this.selectedEventForModal?.id) {
      this.registrationMessage = {
        type: 'error',
        text: 'Per favore, compila tutti i campi e seleziona uno slot.',
      };
    ...
    try {
      await this.eventService.registerVtcToSlot(
        this.selectedEventForModal.id,
        formValues.selectedSlotId,
        bookingInfo
      );
      this.registrationMessage = {
        type: 'success',
        text: "Registrazione all'evento completata con successo!",
      };
      this.vtcRegistrationForm.reset({
        participantsCount: 1,
        selectedSlotId: null,
