...
export class EventService {
  ...
  private generateSubSlots(
    count: number,
    existingSubSlots?: EventSubSlot[]
  ): EventSubSlot[] {
    const newSubSlots: EventSubSlot[] = [];
    for (let i = 0; i < count; i++) {
      const existing = existingSubSlots?.find(
        (es) => es.name === `Postazione ${i + 1}`
      );
      if (existing) {
  ...
  async addEvent(
    eventData: Omit<AppEvent, 'id' | 'createdAt' | 'updatedAt'>
  ): Promise<any> {
    const dataToSave = {
      ...eventData,
      startDate:
      ...
      endDate:
        eventData.endDate instanceof Date
          ...
          : eventData.endDate,
      photoAreaImageUrl:
        eventData.photoAreaImageUrl || this.defaultEventPhotoAreaUrl,
      thumbphotoAreaImageUrl:
        eventData.thumbphotoAreaImageUrl || this.defaultEventPhotoAreaUrl, // Initialize thumbnail field
      routeImageUrl: eventData.routeImageUrl || this.defaultEventRouteUrl,
      thumbrouteImageUrl:
        eventData.thumbrouteImageUrl || this.defaultEventRouteUrl, // Initialize thumbnail field
      slots:
  ...
  async updateEvent(
    ...
    const dataToUpdate: UpdateData<AppEvent> = {
      // Copy all fields from eventData except explicit image URLs and thumbnails
      // We will handle specific image URLs explicitly below
      ...eventData,
      updatedAt: serverTimestamp(),
    };

    // Remove thumb URLs from dataToUpdate if they came from eventData
    // We will explicitly add them back only if they are default or truly provided.
    delete dataToUpdate.thumbphotoAreaImageUrl;
    delete dataToUpdate.thumbrouteImageUrl;
    // We handle slots later for thumbimageUrl
    ...
    if (eventData.endDate && eventData.endDate instanceof Date) {
      ...
      eventData.hasOwnProperty('endDate') &&
      eventData.endDate === null
    ) {
      dataToUpdate.endDate = null;
    }

    // Handle photoAreaImageUrl and its thumbnail
    if (eventData.hasOwnProperty('photoAreaImageUrl')) {
      const newPhotoAreaImageUrl =
        eventData.photoAreaImageUrl === '' ||
        eventData.photoAreaImageUrl === null
          ? this.defaultEventPhotoAreaUrl
          : eventData.photoAreaImageUrl;
      ...
      if (newPhotoAreaImageUrl === this.defaultEventPhotoAreaUrl) {
        ...
        eventData.hasOwnProperty('thumbphotoAreaImageUrl') &&
        eventData.thumbphotoAreaImageUrl !== undefined
      ) {
        // If original is NOT default, and thumbnail is explicitly provided (e.g., from a fresh fetch after CF update)
        dataToUpdate.thumbphotoAreaImageUrl = eventData.thumbphotoAreaImageUrl;
      } else {
        // If original is NOT default, and thumbnail is NOT explicitly provided by Angular,
        // DO NOT include thumbphotoAreaImageUrl in the update. This allows the CF's update to persist.
        // (It was deleted at the start, so it won't be re-added here)
      }
    }

    // Handle routeImageUrl and its thumbnail
    if (eventData.hasOwnProperty('routeImageUrl')) {
      const newRouteImageUrl =
        eventData.routeImageUrl === '' || eventData.routeImageUrl === null
          ? this.defaultEventRouteUrl
          : eventData.routeImageUrl;
      ...
      if (newRouteImageUrl === this.defaultEventRouteUrl) {
        ...
        eventData.hasOwnProperty('thumbrouteImageUrl') &&
        eventData.thumbrouteImageUrl !== undefined
      ) {
        dataToUpdate.thumbrouteImageUrl = eventData.thumbrouteImageUrl;
      } else {
        // DO NOT include thumbrouteImageUrl in dataToUpdate here.
      }
    }

    if (eventData.hasOwnProperty('slots')) {
      const currentEventSnapshot = await getDoc(eventDocRef);
      const currentEventData = currentEventSnapshot.data() as
        | AppEvent
        | undefined;

      dataToUpdate.slots =
        eventData.slots?.map((updatedSlot) => {
          const existingMainSlot = currentEventData?.slots?.find(
            (s) => s.id === updatedSlot.id
          );
          const imageUrl =
            updatedSlot.imageUrl === '' || updatedSlot.imageUrl === null
              ? this.defaultSlotImageUrl
              : updatedSlot.imageUrl;

          const slotUpdate: EventSlot = {
            ...updatedSlot,
            imageUrl: imageUrl,
            subSlots: this.generateSubSlots(
              updatedSlot.numberOfSubSlots,
              existingMainSlot?.subSlots
            ),
          };
          ...
          if (imageUrl === this.defaultSlotImageUrl) {
            ...
            updatedSlot.hasOwnProperty('thumbimageUrl') &&
            updatedSlot.thumbimageUrl !== undefined
          ) {
            slotUpdate.thumbimageUrl = updatedSlot.thumbimageUrl;
          }
          // If imageUrl is NOT default and thumbimageUrl is NOT explicitly provided by Angular,
          // we omit thumbimageUrl from slotUpdate, relying on CF.
          return slotUpdate;
        }) || [];
    }
  ...
  async deleteEvent(eventId: string): Promise<void> {
    ...
    if (eventSnap.exists()) {
      ...
      if (
        eventData.routeImageUrl &&
        eventData.routeImageUrl !== this.defaultEventRouteUrl
      ) {
  ...
  async deleteEventImage(imageUrl: string | null | undefined): Promise<void> {
    ...
    try {
      ...
      console.error(
        `Error deleting image ${imageUrl} from Storage (it might not exist or path is incorrect):`,
        error
      );
    }
  }
  ...
  async registerVtcToEventSubSlot(
    ...
    return runTransaction(this.firestore, async (transaction) => {
      ...
      const eventData = eventSnap.data() as AppEvent;
      const mainSlots = eventData.slots
        ? (JSON.parse(JSON.stringify(eventData.slots)) as EventSlot[])
        : [];
      ...
      const subSlotIndex = targetMainSlot.subSlots.findIndex(
        (ss) => ss.id === subSlotId
      );
      if (subSlotIndex === -1) {
      ...
      if (targetSubSlot.isBooked) {
        throw new Error(
          `La postazione "${targetSubSlot.name}" nella zona "${targetMainSlot.name}" è già prenotata.`
        );
      }
      ...
      try {
        const eventDataForDiscord = {
          ...
          notes:
            eventData.description || "Nessuna nota specifica per l'evento.",
        ...
        console.log(
          'Notifica Discord per iscrizione VTC inviata con successo.'
        );
      } catch (discordError) {
        console.error(
          "Errore durante l'invio della notifica Discord per iscrizione VTC:",
          discordError
        );
      }
    });
  ...
  async sendVtcSubscriptionToDiscord(eventData: any): Promise<any> {
    if (!this.firebaseEventWebhookUrl) {
      console.error(
        'URL del webhook Firebase per eventi non configurato in environment.ts'
      );
    ...
    try {
      return await firstValueFrom(
        this.http.post(this.firebaseEventWebhookUrl, eventData)
      );
    } catch (error) {
